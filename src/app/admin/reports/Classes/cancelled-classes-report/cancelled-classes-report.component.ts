...
import { CancelledClassesReport, ClassesReportFilters } from 'src/app/models/reports.model';
...
@Component({
  selector: 'app-canceled-classes-report',
  templateUrl: './cancelled-classes-report.component.html',
  styleUrls: ['./cancelled-classes-report.component.scss'],
  imports: [ReportsPageHeaderComponent, ClassesReportFilterComponent, DatePipe, TranslateModule]
})
export class CancelledClassesReportComponent implements OnInit {

  filters: ClassesReportFilters = new ClassesReportFilters();
  reports: CancelledClassesReport[];
  isResult: boolean;
  ...
  generate() {
    this.isResult = false;
    this.filters.isCancelled = true;
    this.reportsService.getCanceledClassesReport(this.filters).subscribe({
      next: (res) => {
        this.isResult = true;
