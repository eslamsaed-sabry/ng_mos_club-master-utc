...
import { FinancialFilters, IFinancial, dialogFinancialData } from 'src/app/models/accounts.model';
...
import { SalariesFiltersComponent } from './salaries-filters/salaries-filters.component';
...
export class SalariesComponent implements OnInit {
  ...
  getSalaries() {
    this.filters.type = Accounts.SALARIES;
  ...
  addSalaries() {
    ...
    data.type = 'add';
    let dialogRef = this.dialog.open(SalariesFormComponent, {
      maxHeight: '80vh',
      maxWidth: '500px',
      data: data,
    });
  ...
  editSalaries(currentSalaries: IFinancial) {
    let data = {} as dialogFinancialData;
    data.type = 'edit';
    data.financial = currentSalaries;
    let dialogRef = this.dialog.open(SalariesFormComponent, {
      maxHeight: '80vh',
      maxWidth: '500px',
      data: data,
  ...
  onDeleteSalaries(currentSalaries: IFinancial) {
    let dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      maxHeight: '80vh',
      data: { mainTitle: this.translate.instant('accounts.msgToDeletedSalaries') },
      disableClose: true
    });

    dialogRef.afterClosed().subscribe(result => {
      if (result.status === 'yes') {
        this.deleteSalaries(currentSalaries.id);
      }
    });
  }

  deleteSalaries(salariesID: number) {
    this.accountService.deleteFinancial(salariesID).subscribe({
      next: (res) => {
        if (res) {
          this.getSalaries();
        }
  ...
  getReceiptType(currentSalaries: IFinancial) {
    this.memberService.getGymConfig(GymConfig.receiptType).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res) => {
        currentSalaries.receiptType = res.data;
        this.print(currentSalaries);//Financial
      }
    })
  }

  print(currentSalaries: IFinancial) {
